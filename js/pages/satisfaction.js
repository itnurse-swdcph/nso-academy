  ...
  async _renderTakeForm(container) {
    ...
    try {
      ...
      document.getElementById('satResponseForm').addEventListener('submit', async (e) => {
        ...
        try {
          // สร้าง participantId ชั่วคราว unique ต่อการ submit 1 ครั้ง
          // ใช้ timestamp + random เพื่อให้ระบุกลุ่มคำตอบของแต่ละคนได้
          // ไม่เชื่อมโยงกับข้อมูลส่วนตัวใด ๆ (ไม่มีชื่อ/รหัสพนักงาน)
          const anonParticipantId = `ANON-${Date.now()}-${Math.floor(Math.random()*9999)}`;
          await API.submitSatisfaction({
            trainingId: this._trainingId,
